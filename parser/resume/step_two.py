...
import signal
import numpy as np
from ..resume.entities import degrees, schools, corporations
...
def forEdu(cv):
    ...
    if cv["school_rank_int"] <= 20 or ("海外名校" in fea and cv["school_rank_int"] <= 200):
        ...
        or ("海外名校" in fea and cv["school_rank_int"] <= 500 and cv["school_rank_int"] > 200)
    ):
    ...
    if sch:
        ...
        if (
            (
                ...
                and any([d.lower() in ["硕士", "博士", "mba", "博士"] for d in cv.get("degree_kwd", [])])
            )
...
def parse(cv):
    ...
    for k in keys:
        ...
        if k in kwd_fld:
            cv[f"{k}_kwd"] = [
                n.lower()
                for n in re.split(r"[\t,，；;. ]", re.sub(r"([^a-zA-Z])[ ]+([^a-zA-Z ])", r"\1，\2", cv[k]))
                if n
            ]
