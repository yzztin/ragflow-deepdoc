...
import json
from ..resume.entities import degrees, regions, industries
...
def refactor(df):
        ...
        for c in nms:
            if cc:
                ...
                df[c] = df["obj"].map(
                    ...
                    if isinstance(x, type({})) and (isinstance(x.get(c), type({})) or not x.get(c))
    ...
    df["industry_names"] = df["industry_ids"].map(
        lambda x: " ".join([" ".join(industries.get_names(i)) for i in str(x).split(",")])
    )
